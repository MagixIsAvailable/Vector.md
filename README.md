...
| `VECTOR_OPERATOR` | all three always-on scripts | `unknown` (`vector` inside `brain_proxy.py` specifically, since those requests are his own voice) | **Set this to your own name/handle.** Every logged action records who did it — this is how the multi-agent logging design (see DEVLOG.md) tells "you drove him" from "the n8n schedule did" apart. |
| `VECTOR_OWNER_NAME` | `brain_proxy.py`, `vector_mcp_server.py`, `vector_life.py`, `self_improve.py` | `my human` | **Set this to your actual name.** This is what Vector calls you out loud and in the text he sends the LLM (greetings, asking permission, thanking you, referring to you in his own memory digest). |
| `VECTOR_OWNER_TAG` | `vector_mcp_server.py` (`vector_human_log`, eye colours) | `owner` | Short machine-readable handle for *you* specifically in structured logs (the logbook, `actions.jsonl`) and the `OPERATOR_EYES` colour map — separate from `VECTOR_OWNER_NAME` since log tags read better short (`owner`, `mike`, `alex`) than as a full display name. |
